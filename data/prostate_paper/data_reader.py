...
from data.prostate_paper.true_enhancers import true_enhancers   
...
splits_path = join(INTERACTIONS_PATH, 'splits_p4-6')

# use this one
cnv_filename = 'P1000_data_CNA_paper.csv' #copy number
gene_important_mutations_only = 'P1000_final_analysis_set_cross_important_only.csv' # Mutation

matrix_filename = 'Enh_vs_Genes_matrix_number.txt'
...
def load_data(filename, selected_genes=None):
    filename = join(interactions_path, filename)
...
def load_data_type(data_type='gene', cnv_levels=5, cnv_filter_single_event=True, mut_binary=False, selected_genes=None):
    ...
    if data_type == 'enh_vs_genes':
        x, response, info, genes = load_data(matrix_filename, selected_genes) # info == enh
    
    return x, response, info, genes

def get_response():
    logging.info('loading response from %s' % interactions_filename)
    labels = true_enhancers(interactions_filename, threshold)
    labels = labels.set_index('Enh')
    return labels
...
class ProstateDataPaper():
    ...
    def __init__(self, data_type='mut', account_for_data_type=None, cnv_levels=5,
        ...
        if not selected_genes is None:
            if type(selected_genes) == list:
                ...
                selected_genes_file = join(data_path, 'enh_vs_genes')
    ...
    def get_train_validate_test(self):
        ...
        training_set = pd.read_csv(join(splits_path, 'training_set.csv'))
