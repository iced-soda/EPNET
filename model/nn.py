...
from model.callbacks_custom import GradientCheckpoint, FixedEarlyStopping, MetricsCallback
...
class Model(BaseEstimator):
    ...
    def get_callbacks(self, X_train, y_train, x_val, y_val):
        callbacks = []
        if self.reduce_lr:
    ...
    def get_th(self, y_validate, pred_scores):
        ...
        print(ret)
        ret.to_csv('output_thresholds' + timeStamp + '.csv', index=False)
    ...
    def fit(self, X_train, y_train, X_val=None, y_val=None):
        ...
        callbacks = self.get_callbacks(X_train, y_train, X_val, y_val)
        ...
        with open('all_metrics.csv', 'w', newline='') as csvfile:
            fieldnames = ['epoch', 'accuracy', 'precision', 'recall', 'f1']
            ...
            for epoch, metrics in enumerate(zip(
                    metrics_callback.metrics_history['accuracy'],
                    metrics_callback.metrics_history['precision'],
                    metrics_callback.metrics_history['recall'],
                    metrics_callback.metrics_history['f1']
            ), start=1):
                writer.writerow({'epoch': epoch, 'accuracy': metrics[0], 'precision': metrics[1], 'recall': metrics[2], 'f1': metrics[3]})
